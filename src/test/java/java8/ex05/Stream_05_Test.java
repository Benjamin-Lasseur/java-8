...
public class Stream_05_Test {
	...
	public void test_collectingAndThen() throws IOException {
		...
		try (Stream<String> lines = java.nio.file.Files.lines(Paths.get(NAISSANCES_DEPUIS_1900_CSV))) {
			...
			// TODO construire une MAP (clé = année de naissance, valeur =
			// maximum de nombre de naissances)
			// TODO utiliser la méthode "collectingAndThen" à la suite d'un
			// "grouping"
			Map<String, Naissance> result = lines.skip(1).map(str -> {
				...
				return n;
					...
					collectingAndThen(maxBy(Comparator.comparingInt(Naissance::getNombre)), Optional::get)));
	...
	// TODO Optionel
	// TODO Créer un test qui exporte des données new Data().getPizzas() dans
	// des fichiers
